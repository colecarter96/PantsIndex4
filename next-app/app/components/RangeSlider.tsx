      ...
      <Range
        ...
        renderTrack={({ props, children }) => {
            ...
            <div
                ...
                height: '4px',
                background: `linear-gradient(to right, #4A90E2 ${((values[0] - min) / (max - min)) * 100}%, #2196f3 ${((values[1] - min) / (max - min)) * 100}%)`,
                }}
            >
                {children}
            </div>
        ...
        renderThumb={({ props }) => {
            ...
            <div
                {...thumbProps}
                key={key}
                className="w-4 h-4 bg-blue-500 rounded-full shadow-md cursor-pointer"
            />
            );
        }}
        />
